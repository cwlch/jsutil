...
var extend = function (target) {
    ...
    var last = args[args.length - 1], lastType = get_varType_1.default(last), deep = lastType === 'boolean' ? last : true, soucre = args;
    if (lastType === 'boolean') {
    ...
    if (soucre.length > 1) {
        ...
        for (var key in curSoucre) {
            var me = curSoucre[key], meType = get_varType_1.default(me);
            if (deep && (['array', 'object'].includes(meType))) {
                var _target = null;
                if (['object'].includes(meType)) {
                    _target = ['object'].includes(get_varType_1.default(target[key])) ? target[key] : {};
                }
                else {
                    _target = ['array'].includes(get_varType_1.default(target[key])) ? target[key] : [];
                }
